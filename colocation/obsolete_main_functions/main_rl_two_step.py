...
def parse_args():
    ...
    parser.add_argument('-model', default='stn', type=str,
                        choices=['stn'])
    parser.add_argument('-pipeline', default='two_step', type=str,
...
def joint_visualization(model, test_embedding, train_embedding, n_iter):
...
def dml_visualization(model, train_x, test_x):
    ...
    for fold, test_index in enumerate(test_indexes):
        ...
        for restart in range(5):
            ...
            for epoch in range(config.epoch):
                ...
                if epoch % visualization_step == 0 and visualization:
                    logging("This is %d epoch ......\n" % (epoch + 1))
                    ...
                    model.eval()
                    dml_visualization(model, train_x[:2*k*m], test_x_reserved)
                    model.train()
            ...
            for n_iter in tqdm(range(int(n_epoch * explore_end_at))):
                ...
                if n_iter % visualization_step == 0 and visualization:
                    ...
                    model.eval()
                    joint_visualization(model, RI_test_in, RI_train_in, n_iter)
                    model.train()
                # print(id(model))
        ...
        for n_iter in tqdm(range(n_epoch-int(n_epoch * explore_end_at))):
            ...
            weights = torch.ones(args.num_rooms-10) # number of training rooms
            index = torch.cat([torch.multinomial(weights, k, replacement=False) for _ in range(n_episode)], 0)
