...
def main():
    ...
    for fold, test_index in enumerate(test_indexes):
        ...
        type_constraint = []
        for i in range(k*m):
            for j in range(1, k - i//m):
                type_constraint.append([i, i+m*j])
        print(type_constraint)
        # print(test_g)
        tr = []
        for i in range(k*m):
            tr.append(1)
        adj = test_g.ndata['adj'].cpu().detach().numpy()
        C = -adj
        for i in range(k*m):
            C[i][i] = 0
            for j in range(k*m):
        ...
        print(C)
        X = cp.Variable((k*m, k*m), symmetric=True)
        X_copy = cp.Variable((k*m, k*m), nonneg=True)
        constraints = [X >> 0]
        constraints += [
            X[i][i] == tr[i] for i in range(k*m)
        ]
        constraints += [
            X[exclusive_type[0]][exclusive_type[1]] == 0 for exclusive_type in type_constraint
        ]
        constraints += [
            X == X_copy
        ]
        ...
        # ]
        constraints += [
            cp.sum(X[i]) == m for i in range(k*m)
        ]
        ...
        print(X.value)
        sdp_solution = X.value
        print(np.argsort(sdp_solution, axis=1))
        # edge_info = []
        # for i in range(len(sdp_solution)):
        #     top_m = np.argsort(sdp_solution[i])[-m:]
        #     for ele in top_m:
        #         if [ele, i] in edge_info:
        #             np.argsort(sdp_solution[i])[-(m+1)]
        #         edge_info.append([ele, i])
        # print(edge_info)
        print(np.argsort(sdp_solution, axis=1)[:, -m:])
        print(np.argsort(sdp_solution, axis=1)[-m:, :])
        # constraints += [
