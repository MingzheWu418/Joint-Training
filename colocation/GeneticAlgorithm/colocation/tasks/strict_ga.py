...
from read_data import save_fig_with_result
...
def run(config: config_loader.ColocationConfig):
    ...
    best_solution = None
    # print(corr_func)
    # embedding = MDS(dissimilarity="precomputed").fit_transform(1-corr_matrix)

    assert not np.isnan(population).any(), 'population has nan'
    for iteration in range(config.max_iteration):
        fitnesses: np.ndarray = ga.fitness(population, corr_func) #[[0 1 2 3], [4 5 6 7] .. [.. 39]]
        # print(population)
        ...
        # if iteration % 100 == 0:
        #     print(best_solution)
        #     save_fig_with_result(10, 4, 1, embedding, best_solution, iteration)
